document.addEventListener('DOMContentLoaded', () => {
  ...
  function createNotificationCard(note) {
    ...
    card.className = 'group flex items-start gap-3 p-4 pl-6 rounded-lg bg-white hover:bg-sky-100 transition-colors relative';
...
function renderNotificationList(container, data) {
  ...
  btnWrapper.className = 'flex justify-end gap-2 mb-4';

  // Mark All As Read button
  const markAllReadBtn = document.createElement('button');
  markAllReadBtn.className = 'text-xs text-sky-600 hover:text-sky-700 px-3 py-1 rounded transition-colors';
  ...
  markAllReadBtn.addEventListener('click', () => {
    ...
    fetch('../actions/mark_all_notifications_read.php', {
      method: 'POST',
      headers: { 'Content-Type': 'application/x-www-form-urlencoded' }
    })
      .then(res => res.text())
      .then(text => {
        const response = safeJsonParse(text);
        if (response?.success) {
          cachedNotifications = cachedNotifications.map(n => ({ ...n, status: 'read' }));
          fetchNotifications();
          updateBellIndicator();
        } else {
          console.error('Mark all as read failed:', response?.error || 'Unknown error');
        }
      })
      .catch(err => {
        console.error('Mark all as read request failed:', err);
      });
  });

  btnWrapper.appendChild(markAllReadBtn);
