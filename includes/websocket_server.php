...
class Chat implements MessageComponentInterface
{
    ...
    private function handleSendMessage(ConnectionInterface $from, array $data): void
    {
        ...
        $createdAt = date('Y-m-d H:i:s'); // Approximate; or query NOW()
    ...
    private function getSenderInfo(int $senderId, string $senderType): array
    {
        ...
        if ($result) {
            ...
            if ($photoFile) {
                $photo = ($senderType === 'admin')
                    ? getAdminAvatar(['admin_photo' => $photoFile])
                    : getClientAvatar(['client_profile_photo' => $photoFile]);
            }
        }
