...
<!-- 👤 Client Contact Details Card - WOW Factor Design -->
<div class="relative overflow-hidden rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-300 hover:scale-[1.02] h-full flex flex-col">
  ...
  <!-- Content -->
  <div class="relative z-10 p-6 space-y-4 flex-1">
    <!-- Client Info: Avatar, Name & Status -->
    <div class="flex items-center gap-4 pb-4 border-b border-white/30">
      <!-- Avatar -->
      <img 
        src="<?= !empty($client['client_profile_photo']) ? '../uploads/client_profiles/' . htmlspecialchars($client['client_profile_photo']) : '../images/default_client_profile.png' ?>" 
        alt="<?= htmlspecialchars($fullName) ?>"
        class="bg-white w-16 h-16 rounded-full object-cover border-4 border-white/100 shadow-lg flex-shrink-0"
        loading="lazy"
      />
      <!-- Name & Status -->
      <div class="flex-1 min-w-0">
        <h3 class="pb-1 truncate text-2xl font-bold text-white break-words max-w-[80%]"><?= htmlspecialchars($fullName) ?></h3>
        <?php 
        ...
        ?>
        <span class="inline-block px-3 py-1 rounded-full bg-white/10 border border-white/20 text-white text-xs font-semibold <?= $badgeClass ?> shadow-md">
    ...
    <!-- Dropdown Menu -->
    <div class="absolute top-4 right-4 z-50" x-data="{ open: false }" @click.outside="open = false">
      <button 
        @click="open = !open"
        class="p-2.5 bg-white/90 hover:bg-white rounded-full shadow-lg transition backdrop-blur-sm border border-gray-200"
      ...
      <div x-show="open" x-transition x-cloak
        ...
        <button 
          ...
          class="w-full text-left px-5 py-3.5 text-sm font-medium text-gray-800 hover:bg-sky-50 transition"
        >
          Update Client
        </button>
        <button 
          @click="$store.modals.archiveClient = true; open = false"
          class="w-full text-left px-5 py-3.5 text-sm font-medium text-red-600 hover:bg-red-50 transition"
        >
          Archive Client
        </button>
      </div>
    ...
    <div class="grid grid-cols-2 gap-4 pt-2">
      ...
      <div class="space-y-2">
        ...
        <?php if ($email): ?>
          <a href="mailto:<?= htmlspecialchars($email) ?>"
             class="text-base font-bold text-white hover:text-sky-200 transition block truncate" title="<?= htmlspecialchars($email) ?>">
            <?= htmlspecialchars($email) ?>
          </a>
        <?php else: ?>
          <p class="text-base font-bold text-white/50 italic">Not provided</p>
        <?php endif; ?>
      </div>
      ...
      <div class="space-y-2">
        ...
        <?php if ($phone): ?>
          <a href="tel:<?= htmlspecialchars($phone) ?>"
             class="text-base font-bold text-white hover:text-sky-200 transition block truncate">
      ...
      <div class="space-y-2">
        ...
        <p class="text-base font-bold text-white font-mono"><?= htmlspecialchars($accessCode) ?></p>
      </div>
        ...
        <?php if ($hasPassport): ?>
          <div class="space-y-1">
            <?php if ($passportNumber): ?>
              <p class="text-base font-bold text-white break-all"><?= htmlspecialchars($passportNumber) ?></p>
            <?php endif; ?>
            <?php if ($passportExpiry): ?>
              <p class="text-xs text-amber-300">Expires <?= $passportExpiry ?></p>
            <?php endif; ?>
          </div>
        <?php else: ?>
          <p class="text-base font-bold text-white/50 italic">Not provided</p>
