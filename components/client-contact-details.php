...
<div class="relative overflow-hidden rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-300 sm:hover:scale-[1.02] h-full flex flex-col">
  ...
  <div class="relative z-10 p-4 sm:p-6 space-y-4 flex-1">
    ...
    <div class="flex flex-col sm:flex-row items-center sm:items-center gap-3 sm:gap-4 pb-4 border-b border-white/30">
      ...
      <div class="flex-1 min-w-0 text-center sm:text-left w-full sm:w-auto">
        <h3 class="pb-1 text-xl sm:text-2xl font-bold text-white break-words"><?= htmlspecialchars($fullName) ?></h3>
        <?php 
    ...
    <div class="absolute top-3 right-3 sm:top-4 sm:right-4 z-50" x-data="{ open: false }" @click.outside="open = false">
      <button 
        @click="open = !open"
        class="p-2 sm:p-2.5 bg-white/90 hover:bg-white rounded-full shadow-lg transition backdrop-blur-sm border border-gray-200"
        aria-label="Toggle client actions"
      >
        <svg class="w-4 h-4 sm:w-5 sm:h-5 text-gray-700" fill="currentColor" viewBox="0 0 20 20">
          <path d="M10 6a2 2 0 110-4 2 2 0 010 4zm0 6a2 2 0 110-4 2 2 0 010 4zm0 6a2 2 0 110-4 2 2 0 010 4z" />
        </svg>
      </button>

      <div x-show="open" x-transition x-cloak
           class="absolute right-0 mt-2 w-48 sm:w-56 bg-white rounded-xl shadow-2xl border border-gray-200 overflow-hidden z-50">
        <button 
          @click="$store.modals.editClient = true; open = false"
          class="w-full text-left px-4 sm:px-5 py-3 sm:py-3.5 text-sm font-medium text-gray-800 hover:bg-sky-50 transition"
        >
          Update Guest Info
        </button>
        <button 
          @click="$store.modals.archiveClient = true; open = false"
          class="w-full text-left px-4 sm:px-5 py-3 sm:py-3.5 text-sm font-medium text-red-600 hover:bg-red-50 transition"
        >
          Archive Guest
        </button>
      </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 pt-2">
      ...
      <div class="space-y-2 min-w-0">
        ...
        <?php if ($email): ?>
          <a href="mailto:<?= htmlspecialchars($email) ?>"
             class="text-sm sm:text-base font-bold text-white hover:text-sky-200 transition block break-all" title="<?= htmlspecialchars($email) ?>">
      ...
      <div class="space-y-2 min-w-0">
        ...
        <?php if ($phone): ?>
          <a href="tel:<?= htmlspecialchars($phone) ?>"
             class="text-sm sm:text-base font-bold text-white hover:text-sky-200 transition block truncate">
      ...
      <div class="space-y-2 min-w-0">
        ...
        <p class="text-sm sm:text-base font-bold text-white font-mono"><?= htmlspecialchars($accessCode) ?></p>
      </div>
