...
<div x-show="$store.archivedVisaModal.isOpen" 
     x-cloak
     @keydown.escape.window="$store.archivedVisaModal.close()"
     class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4"
     style="display: none;">
  
  <div @click.away="$store.archivedVisaModal.close()" 
       class="bg-white rounded-lg shadow-xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
    
    <!-- Header -->
    <div class="flex items-center justify-between px-6 py-4 border-b border-gray-200">
      <h3 class="text-lg font-semibold text-gray-900">Archived Visa Packages</h3>
      <button @click="$store.archivedVisaModal.close()" 
    ...
    <!-- Loading State -->
    <div x-show="$store.archivedVisaModal.loading" class="flex items-center justify-center py-12">
      <div class="animate-spin rounded-full h-8 w-8 border-b-2 border-sky-600"></div>
    </div>

    <!-- Content -->
    <div x-show="!$store.archivedVisaModal.loading" class="flex-1 overflow-y-auto p-6">
      
      <!-- Empty State -->
      <template x-if="$store.archivedVisaModal.packages.length === 0">
        <div class="flex flex-col items-center justify-center py-12 text-gray-500">
          ...
          <p class="text-sm text-gray-600">All visa packages are active.</p>
        </div>
      </template>

      <!-- Package List -->
      <div class="space-y-2">
        <template x-for="pkg in $store.archivedVisaModal.packages" :key="pkg.id">
            ...
            <div class="flex flex-col sm:flex-row gap-2 p-2">
              ...
              <div class="max-w-[70%]">
                ...
                <div class="flex items-center gap-2 mt-1 text-xs text-gray-500">
                  <span class="inline-block bg-gray-100 px-1.5 py-0.5 rounded text-xs" x-text="pkg.country"></span>
                  <span x-text="`${pkg.processing_days} days processing`"></span>
                </div>
              </div>

              <!-- Actions -->
              <div class="flex gap-2 ml-auto">
                <button @click="$store.archivedVisaModal.unarchive(pkg.id)" 
                ...
                <button @click="$store.archivedVisaModal.deletePermananently(pkg.id, pkg.name)" 
