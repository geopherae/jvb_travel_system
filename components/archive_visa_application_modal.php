...
<div x-show="$store.modals.archiveVisaClient" x-cloak
     class="fixed inset-0 bg-black/50 z-50 flex items-center justify-center px-4 backdrop-blur-sm"
     @click.outside="$store.modals.archiveVisaClient = false"
     @keydown.escape.window="$store.modals.archiveVisaClient = false">
  <div class="bg-white rounded-xl shadow-xl border border-gray-200 p-6 max-w-md w-full space-y-5">
    ...
    <form action="../actions/archive_visa_application.php" method="POST" class="space-y-4">
      ...
      <input type="hidden" name="application_id" :value="$store.modals.applicationId || 0">

      <div class="flex justify-end gap-3 pt-2">
        <button type="button"
                @click="$store.modals.archiveVisaClient = false"
        ...
        <button type="submit"
                ...
                :disabled="!$store.modals.applicationId">
