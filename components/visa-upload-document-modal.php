...
<div x-cloak x-transition x-show="modals.uploadDocument" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-3 sm:p-4">
    <div @click.away="modals.uploadDocument = false"
      ...
      <div class="flex items-center justify-between gap-3 sm:gap-4">
        <h2 class="text-base sm:text-lg font-semibold text-gray-800">Upload Document</h2>
...
<form action="../actions/submit_visa_document.php" method="POST" enctype="multipart/form-data" class="space-y-4 sm:space-y-6" @submit.prevent="handleUploadDocument">
  ...
  <div class="grid grid-cols-1 md:grid-cols-5 gap-4 md:gap-6">
    ...
    <div class="md:col-span-2">
      <label for="upload_document_file" class="block text-xs sm:text-sm font-medium text-gray-700 mb-2 sm:mb-3">Upload File <span class="text-red-500">*</span></label>
    ...
    <!-- RIGHT COLUMN: Requirement Description -->
    <div class="md:col-span-3">
      <div class="h-full flex flex-col">
        <label class="block text-xs sm:text-sm font-medium text-gray-700 mb-2 sm:mb-3">Requirement Description</label>
        <div class="flex-1 p-3 sm:p-4 bg-gradient-to-br from-gray-50 to-gray-100 rounded-lg border border-gray-200 min-h-[120px] sm:min-h-[160px]">
          <p class="text-xs sm:text-sm text-gray-700 leading-relaxed" x-text="selectedRequirementDescription || '—'"></p>
        </div>
      </div>
    </div>
  </div>
