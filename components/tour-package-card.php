...
<div class="relative overflow-hidden rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-300 hover:scale-[1.02] h-full flex flex-col">
  ...
  <!-- Content -->
  <div class="relative z-10 p-6 space-y-4 flex-1 flex flex-col">
    <!-- Header: Package Name Only -->
    <div>
      <p class="text-[0.65rem] font-semibold text-sky-100 uppercase tracking-wider mb-1">Assigned Package</p>
      <h3 class="pb-3 truncate text-2xl font-bold text-white break-words max-w-[80%]">
    ...
    <!-- Dropdown Menu -->
    <div x-data="{ open: false }" class="absolute top-4 right-4 z-50">
      <button 
        @click="open = !open"
      ...
      <div x-show="open" x-transition x-cloak @click.outside="open = false"
        ...
        <div x-data="{ showTip: false }" class="relative">
          <button type="button"
                  ...
                  class="w-full text-left px-5 py-3.5 text-sm font-medium <?= $hasPackage ? 'text-gray-800 hover:bg-sky-50' : 'text-gray-400 cursor-not-allowed' ?> transition"
                  <?= $hasPackage ? '' : 'disabled' ?>
        ...
        <div x-data="{ showUnassignTip: false }" class="relative">
          <button type="button"
                  ...
                  class="w-full text-left px-5 py-3.5 text-sm font-medium <?= $hasPackage ? 'text-red-600 hover:bg-red-50' : 'text-gray-400 cursor-not-allowed' ?> transition"
                  <?= $hasPackage ? '' : 'disabled' ?>
    ...
    <div class="grid grid-cols-2 gap-4 pt-4 border-t border-white/30 mt-auto">
      <!-- Trip Dates -->
      <div class="space-y-2">
        ...
        <p class="text-base font-bold text-white"><?= $tripDateRangeDisplay ?></p>
      </div>
      ...
      <div class="space-y-2">
        ...
        <p class="text-base font-bold text-white font-mono <?= empty($client['booking_number']) ? 'italic opacity-70' : '' ?>">
      ...
      <div class="space-y-2">
        ...
        <p class="text-base font-bold text-white"><?= $durationDisplay ?></p>
      </div>
      ...
      <div class="space-y-2">
        ...
        <p class="text-base font-bold text-white truncate" title="<?= htmlspecialchars($assignedAgent) ?>"><?= htmlspecialchars($assignedAgent) ?></p>
      </div>
    </div>
  </div>
</div>
