...
$visaClients = $visaClients ?? [];
$tableTitle = 'Visa Processing Clients';

// 🧠 Detect if this is an AJAX reload or a clean reload
$isAjaxReload = basename($_SERVER['SCRIPT_NAME']) === 'reload_visa_clients_table.php';
...
if (!function_exists('getVisaStatusBadgeClass')) {
...
// 🔃 Apply status sort
if ($sort === 'status') {
  usort($visaClients, function ($a, $b) use ($order) {
    $priorityA = getVisaStatusPriority($a['visa_status'] ?? '');
    $priorityB = getVisaStatusPriority($b['visa_status'] ?? '');
...
$limit = 6;
$offset = ($page - 1) * $limit;
$totalClients = count($visaClients);
$paginatedClients = array_slice($visaClients, $offset, $limit);
$totalPages = ceil($totalClients / $limit);

// Debug: Log visa client data
error_log("DEBUG: visa-clients-table.php - Total visa clients: $totalClients, Paginated clients: " . count($paginatedClients));
?>

<div id="visa-clients-table-container">
...
<section class="bg-white p-4 md:p-6 rounded-lg shadow-sm border border-gray-200">
  ...
  <div class="flex items-center justify-between gap-2 mb-4">
    <div class="flex items-center gap-2">
      ...
      <button
        type="button"
        onclick="alert('Refresh function not yet implemented')"
...
<?php if (empty($visaClients)): ?>
  <div class="flex flex-col items-center justify-center py-12 text-gray-500">
    <?php echo getEmptyStateSvg('no-clients-found'); ?>
    <h3 class="text-base italic font-semibold text-sky-700 mb-1">No Visa Clients Yet</h3>
    <p class="text-sm italic text-sky-700 text-center max-w-sm">
      You haven't added any visa processing clients yet. Use the <strong>Add New Visa Client</strong> button above to get started.
    </p>
  </div>
<?php else: ?>
      ...
      >
        ...
        <td class="p-2 md:p-4 text-center">
          <span class="px-2 md:px-3 py-1 text-xs font-semibold rounded-full <?= getVisaStatusBadgeClass($statusText) ?>">
            <?= htmlspecialchars(ucwords(str_replace('_', ' ', $statusText))) ?>
          </span>
        </td>
        <td class="p-2 md:p-4 text-center">
          <button type="button"
