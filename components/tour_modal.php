...
<div
  ...
  x-cloak
  class="backdrop-blur-sm fixed inset-0 z-[50] flex items-center justify-center bg-black/55 px-4"
...
>
  <!-- Modal Container -->
  <div
    class="relative bg-white rounded-lg shadow-xl max-w-5xl w-full overflow-hidden transition-all duration-300 max-h-[95vh] overflow-y-auto"
  ...
  >
    ...
    <div class="w-full flex flex-col sm:flex-row gap-0 sm:gap-4 sm:p-6">
      <!-- Left Column -->
      ...
      <!-- Right Column -->
      <div class="sm:max-w-[45%] w-full flex flex-col border-t sm:border-t-0 sm:border-l border-slate-100 px-5 py-5 sm:py-0 sm:pl-6">
        <!-- Tabs -->
        <div class="flex border-b mb-4 gap-3">
          <button
            type="button"
...
<div x-show="$store.tourModal.tab === 'itinerary'" 
     class="flex-1 pr-1 max-h-[300px] overflow-y-auto space-y-5 text-left"
     x-data="{ openDay: null }">
  ...
  <template x-if="$store.tourModal.activeTour.itinerary && $store.tourModal.activeTour.itinerary.length">
    ...
    <!-- Divider -->
    <div class="border-t border-slate-200 mt-6 pt-4"></div>

    <!-- Action Buttons -->
    <div class="flex justify-end gap-6 px-5 pb-5">
      <button
        type="button"
      ...
      <button
        type="button"
        class="text-sm text-red-600 border border-red-600 px-4 py-2 rounded hover:bg-red-600 hover:text-white transition"
